...
class TestMoldTypeModelViewSet(APITestCase):
    ...
    def test_create_with_only_user(self):
        response = self.client.post(reverse('situation-list'), {
            'name': 'any name',
        })
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        obj = Situation.objects.get(uuid=response.data['uuid'])
    ...
    def test_get_with_user(self):
        obj = SituationFactory(user=self.user)
        response = self.client.get(reverse('situation-detail', kwargs={'pk': str(obj.uuid)}))
    ...
    def test_get_with_entity(self):
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + str(self.user_with_entity_token))
        obj = SituationFactory(user=self.user, entity=self.entity)
        response = self.client.get(reverse('situation-detail', kwargs={'pk': str(obj.uuid)}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        entity = response.data
        self.assertTrue('uuid' in entity)
    ...
    def test_update_with_user_with_patch(self):
        obj = SituationFactory(user=self.user)
        response = self.client.patch(reverse('situation-detail', kwargs={'pk': str(obj.uuid)}), {
            'name': 'ABC'
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        obj.refresh_from_db()
    ...
    def test_update_with_user_with_put(self):
        obj = SituationFactory(user=self.user)
        response = self.client.put(reverse('situation-detail', kwargs={'pk': str(obj.uuid)}), {
            'name': obj.name,
            'enabled': obj.enabled
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_delete_with_user(self):
        obj = SituationFactory(user=self.user)
        self.assertEqual(1, Situation.objects.filter(uuid=obj.uuid).count())
        response = self.client.delete(reverse('situation-detail', kwargs={'pk': str(obj.uuid)}))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(0, Situation.objects.filter(uuid=obj.uuid).count())
