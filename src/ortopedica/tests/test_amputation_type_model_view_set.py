...
class TestAmputationTypeModelViewSet(APITestCase):
    def setUp(self):
    ...
    def test_create_with_only_user(self):
        response = self.client.post(reverse('amputation-type-list'), {
            'name': 'any name',
        })
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        obj = AmputationType.objects.get(uuid=response.data['uuid'])
    ...
    def test_get_with_user(self):
        obj = AmputationTypeFactory(user=self.user)
        response = self.client.get(reverse('amputation-type-detail', kwargs={'pk': str(obj.uuid)}))
    ...
    def test_get_with_entity(self):
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + str(self.user_with_entity_token))
        obj = AmputationTypeFactory(user=self.user, entity=self.entity)
        response = self.client.get(reverse('amputation-type-detail', kwargs={'pk': str(obj.uuid)}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        entity = response.data
        self.assertTrue('uuid' in entity)
    ...
    def test_update_with_user_with_patch(self):
        obj = AmputationTypeFactory(user=self.user)
        response = self.client.patch(reverse('amputation-type-detail', kwargs={'pk': str(obj.uuid)}), {
            'name': 'ABC'
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        obj.refresh_from_db()
    ...
    def test_update_with_user_with_put(self):
        obj = AmputationTypeFactory(user=self.user)
        response = self.client.put(reverse('amputation-type-detail', kwargs={'pk': str(obj.uuid)}), {
            'name': obj.name,
            'enabled': obj.enabled
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_delete_with_user(self):
        obj = AmputationTypeFactory(user=self.user)
        self.assertEqual(1, AmputationType.objects.filter(uuid=obj.uuid).count())
        response = self.client.delete(reverse('amputation-type-detail', kwargs={'pk': str(obj.uuid)}))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(0, AmputationType.objects.filter(uuid=obj.uuid).count())
